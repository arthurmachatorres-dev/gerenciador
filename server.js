...
const pool = require('./db'); // usa o db.js atualizado com variáveis de ambiente
...
app.post('/salario', (req, res) => {
    ...
    pool.query(sql, [mes, ano, salario], (err) => {
...
app.get('/salario/:mes/:ano', (req, res) => {
    ...
    pool.query('SELECT salario FROM salarios WHERE mes=? AND ano=?', [mes, ano], (err, rows) => {
        if (err) return res.status(500).json({ error: err.message });
...
app.post('/contas', (req, res) => {
    ...
    pool.query(sql, [nome, tipo || null, valor, data_vencimento || null, mes, ano], (err) => {
        if (err) return res.status(500).json({ error: err.message });
...
app.get('/contas/:mes/:ano', (req, res) => {
    ...
    const sql = `
      ...
      ORDER BY id DESC
    `;
    pool.query(sql, [mes, ano], (err, results) => {
        if (err) return res.status(500).json({ error: err.message });
        res.json(results);
    });
});
...
app.put('/contas/:id', (req, res) => {
    ...
    pool.query(sql, [nome, tipo || null, valor, data_vencimento || null, status || 'PENDENTE', mes, ano, id], (err) => {
        if (err) return res.status(500).json({ error: err.message });
...
app.put('/contas/:id/status', (req, res) => {
    ...
    pool.query('UPDATE contas SET status=? WHERE id=?', [status, id], (err) => {
        if (err) return res.status(500).json({ error: err.message });
...
app.delete('/contas/:id', (req, res) => {
    const { id } = req.params;
    pool.query('DELETE FROM contas WHERE id=?', [id], (err) => {
        if (err) return res.status(500).json({ error: err.message });
...
app.get('/resumo/:mes/:ano', (req, res) => {
    ...
    pool.query(sqlContas, [mes, ano], (err, contaRows) => {
        if (err) return res.status(500).json({ error: err.message });

        pool.query('SELECT salario FROM salarios WHERE mes=? AND ano=?', [mes, ano], (err, salRows) => {
            if (err) return res.status(500).json({ error: err.message });
